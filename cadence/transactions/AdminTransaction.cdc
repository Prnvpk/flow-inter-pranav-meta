...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &pranavtoken.Vault{pranavtoken.CollectionPublic}
    let signerVault: &pranavtoken.Vault
    let senderFlowVault: &FlowToken.Vault{FungibleToken.Balance, FungibleToken.Receiver, FungibleToken.Provider}
    let adminResource: &pranavtoken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&pranavtoken.Admin>(from: /storage/AdminStorage)
            ?? panic("Admin Resource is not present")

        self.signerVault = acct.borrow<&pranavtoken.Vault>(from: /storage/VaultStorage)
            ?? panic("Vault not found in signerAccount")

        self.senderVault = getAccount(senderAccount)
            .getCapability(/public/Vault)
            .borrow<&pranavtoken.Vault{pranavtoken.CollectionPublic}>()
