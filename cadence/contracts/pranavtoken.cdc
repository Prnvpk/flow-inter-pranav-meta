...
// pranavtoken contract: Implements fungible token on Flow blockchain
pub contract pranavtoken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @pranavtoken.Vault
        ...
        // Destroy the vault
        destroy() {
            pranavtoken.totalSupply = pranavtoken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            pranavtoken.totalSupply = pranavtoken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&pranavtoken.Minter>(/public/Minter, target: /storage/MinterStorage)
